.zcurve_EM_PL          <- function(z, lb, ub, control){

  # fit final z-curve

  fit  <- .zcurve_EM_PL_RCpp        (x          = z,
                                     mu         = control$mu,
                                     sigma      = control$sigma,
                                     theta      = control$theta_alpha,
                                     a          = control$a,
                                     b          = control$b,
...
.zcurve_EM_PL.control  <- function(control){
  #if(is.null(control)){
  ...
  if(is.null(control)){
    ...
    control$theta_alpha     <- rep(.5, length(control$mu))
    control$mu_alpha        <- 2:(control$K+1) # legacy
    control$mu_max          <- control$b + 2
    control$criterion       <- 1e-2
    control$max_iter        <- 1000
    control$criterion_boot  <- 1e-2
    control$max_iter_boot   <- 1000
    control$criterion_start <- 1e-3 # legacy
    control$max_iter_start  <- 100 # legacy
    control$fit_reps        <- 100 # legacy
    control$model           <- "EM" # legacy
    return(control)
  }
  ...
  if(is.null(control[["theta_alpha"]])){
    control$theta_alpha     <- rep(0.5, length(control$mu))
  }
